import { buildContractConfirmation, type ContractConfirmation } from './contract-confirmation';
...
export { paidTierLabel } from './contract-confirmation';

export type OrderConfirmation = ContractConfirmation & { to: string };
 ...
 * than swallowing.
 *
 * The text itself lives in lib/contract-confirmation.ts, because email is not
 * the only medium it goes out on — see the note at the top of that module.
 */
export async function sendOrderConfirmation(o: OrderConfirmation): Promise<boolean> {
  const { subject, text } = buildContractConfirmation(o);
  return sendMail({
    to: o.to,
